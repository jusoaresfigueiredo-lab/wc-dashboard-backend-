...
app.get("/auth/monday", (req, res) => {
  ...
  req.session.save(() => {
    const params = new URLSearchParams({
      client_id: MONDAY_CLIENT_ID, redirect_uri: MONDAY_REDIRECT, state: userId,
    });
...
app.get("/auth/monday/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(`${FRONTEND_URL}?success=monday&user=${userId}`);
  } catch {
    res.redirect(`${FRONTEND_URL}?error=monday_failed`);
  }
});
...
app.get("/status/:userId", (req, res) => {
  ...
  res.json({ userId, hasGoogle: !!getToken(userId, "google"), hasMonday: !!getToken(userId, "monday") });
});
...
function buildMCPServers(googleToken, mondayToken) {
  const servers = [];
  if (googleToken) {
    servers.push({ type: "url", name: "gmail-mcp", url: "https://gmailmcp.googleapis.com/mcp/v1", authorization_token: googleToken.access_token });
    servers.push({ type: "url", name: "google-calendar-mcp", url: "https://calendarmcp.googleapis.com/mcp/v1", authorization_token: googleToken.access_token });
    servers.push({ type: "url", name: "google-drive-mcp", url: "https://drivemcp.googleapis.com/mcp/v1", authorization_token: googleToken.access_token });
  }
  if (mondayToken) {
    servers.push({ type: "url", name: "monday-mcp", url: "https://mcp.monday.com/mcp", authorization_token: mondayToken.access_token });
  }
  return servers;
}
...
topThreads max 4, meetings max 5.`, buildMCPServers(googleToken, null));
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get("/data/team-projects", async (req, res) => {
  const mondayUser = getAllConnectedUsers().find(u => u.hasMonday);
...
Max 8 projects.`, buildMCPServers(null, getToken(mondayUser.userId, "monday")));
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get("/data/team-files", async (req, res) => {
  const googleUser = getAllConnectedUsers().find(u => u.hasGoogle);
  if (!googleUser) return res.status(401).json({ error: "No Google account connected yet" });
  try {
    let googleToken = await refreshGoogleToken(googleUser.userId, getToken(googleUser.userId, "google"));
...
Max 8 files, newest first.`, buildMCPServers(googleToken, null));
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/data/ai-summary", async (req, res) => {
  ...
  try {
    let googleToken = googleUser ? await refreshGoogleToken(googleUser.userId, getToken(googleUser.userId, "google")) : null;
...
Team: ${TEAM_NAMES}. Pull from Gmail, Google Calendar, monday.com, and Google Drive.
Respond with ONLY raw JSON:
...
Be specific — name real projects, suppliers, clients.`, buildMCPServers(googleToken, mondayToken));
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
